			...
			ScrollTrigger.create({
				...
				onEnter: () => {
					gsap.to(targets, {
						...
						ease: "power2.out",
						stagger: kind === "stagger" ? num(el.dataset.revealStagger, 0.12) : 0,
						overwrite: "auto",
					});
				},
			});
...
function initHero(): void {
	...
	const logoScene = pinTarget.querySelector<HTMLElement>("[data-hero-logo-scene]");
	const logo = pinTarget.querySelector<HTMLElement>("[data-hero-logo]");
	const copy = pinTarget.querySelector<HTMLElement>("[data-hero-copy]");
	if (!logoScene || !logo || !copy) return;
	...
	tl.fromTo(logoScene, { scale: 0.92 }, { scale: 1.06, duration: 0.8 }, 0);
	tl.to(logo, { rotation: 14, scale: 1.12, duration: 0.42 }, 0.38);
	tl.to(copy, { scale: 1.025, duration: 0.42 }, 0.38);
	tl.to(logoScene, { yPercent: -10, duration: 0.2 }, 0.8);
	tl.to(copy, { yPercent: -14, autoAlpha: 0.75, duration: 0.2 }, 0.8);
}
...
function initSingleWordCycler(el: HTMLElement): void {
	...
	gsap.set(el, { width: measure(words[index]?.text ?? ""), position: "relative" });
	gsap.set(active, { position: "absolute", left: 0, top: 0, whiteSpace: "nowrap" });
